...
int main(int argc, char **argv)
{
  ...
  */
  ...
  float *com_gpu_w;
  int *lock_gpu;
  cudaMalloc<float>(&com_gpu_x, sizeof(float)); 
  cudaMalloc<float>(&com_gpu_y, sizeof(float)); 
  cudaMalloc<float>(&com_gpu_z, sizeof(float)); 
  cudaMalloc<float>(&com_gpu_w, sizeof(float)); 
  cudaMalloc<int>(&lock_gpu, sizeof(int)); 

  cudaMemset(com_gpu_x, 0, sizeof(float));
  cudaMemset(com_gpu_y, 0, sizeof(float));
  cudaMemset(com_gpu_z, 0, sizeof(float));
  cudaMemset(com_gpu_w, 0, sizeof(float));
  cudaMemset(lock_gpu, 0, sizeof(float));


  sharedMemSize = 4 * sizeof(float) * (red_thr_blc / 32);
  centerOfMass<<<reductionGrid, red_thr_blc, sharedMemSize>>>(particles_gpu[numberOfSteps % 2],
                                                               com_gpu_x,
                                                               com_gpu_y, 
                                                               com_gpu_z,
                                                               com_gpu_w,
  ...
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  cudaMemcpy(&comOnGPU.x, com_gpu_x, sizeof(float), cudaMemcpyDeviceToHost);
  cudaMemcpy(&comOnGPU.y, com_gpu_y, sizeof(float), cudaMemcpyDeviceToHost);
  cudaMemcpy(&comOnGPU.z, com_gpu_z, sizeof(float), cudaMemcpyDeviceToHost);
  cudaMemcpy(&comOnGPU.w, com_gpu_w, sizeof(float), cudaMemcpyDeviceToHost);
